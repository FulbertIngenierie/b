...
func update_posture(_delta):
	# Note: La hauteur du personnage n'est plus ajustée dynamiquement
	# La posture est gérée par les animations
	pass
...
func shoot_at_player():
	...
	# Parfois lancer une grenade au lieu de tirer
	if randf() < 0.1:
		throw_grenade()
	...
	# Animation de tir
	if anim_player:
		# Choisir une animation de tir aléatoire
		var shoot_anims = ["tir ", "tir avant ", "tir_arriere"]
		var random_shoot = shoot_anims[randi() % shoot_anims.size()]
		
		if anim_player.has_animation(random_shoot):
			anim_player.speed_scale = 1.0
			anim_player.stop()
			anim_player.play(random_shoot)
			current_anim = random_shoot
		elif anim_player.has_animation("tir "):
			anim_player.speed_scale = 1.0
			anim_player.stop()
			anim_player.play("tir ")
			current_anim = "tir "
...
func throw_grenade():
	if anim_player and anim_player.has_animation("grenade"):
		anim_player.play("grenade")
		anim_player.speed_scale = 1.0
		current_anim = "grenade"
	
	# Créer une grenade (simulé pour l'instant)
	if player:
		var grenade_direction = (player.global_position - global_position).normalized()
		var grenade_pos = global_position + Vector3(0, 1.5, 0) + grenade_direction * 0.5
		
		# Appliquer des dégâts de zone après un délai
		if is_inside_tree() and get_tree():
			await get_tree().create_timer(2.0).timeout
			var distance = grenade_pos.distance_to(player.global_position)
			if distance < 10.0:
...
func reload():
	...
	if anim_player:
		if anim_player.has_animation("recharge"):
			anim_player.play("recharge")
...
func take_damage(amount):
	...
	# Animation gun_effet quand l'ennemi prend des dégâts
	if anim_player and anim_player.has_animation("gun_effet"):
		anim_player.play("gun_effet")
		anim_player.speed_scale = 1.0
		current_anim = "gun_effet"
